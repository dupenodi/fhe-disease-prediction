...
if __name__ == "__main__":
    (X_train, y_train), (X_test, y_test) = federated_utils.load_disease_dataset()

    # Split train set into 5 partitions and randomly use one for training.
    partition_id = np.random.choice(6)
    (X_train, y_train) = federated_utils.partition(X_train, y_train, 6)[partition_id]
    ...
    class DiseaseClient(fl.client.NumPyClient):
    ...
    # Start Flower client
    fl.client.start_numpy_client(server_address="0.0.0.0:8080", client=DiseaseClient())